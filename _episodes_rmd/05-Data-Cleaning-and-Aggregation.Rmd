...
ab_line <- st_read("abline.gpkg")
abline_utm <- st_transform_utm(ab_line)

design_grids_utm <- make_grids(boundary_grid_utm, abline_utm, long_in = 'NS', short_in = 'EW', length_ft = 30, width_ft = 30)
...
yield_clean <- st_transform_utm(yield_clean)
```
...
```{r save yield}
st_write(yield_clean, "data/yield_clean.gpkg", layer_options = 'OVERWRITE=YES', update = TRUE)
```
...
yield_sp <- as(yield_clean[,"Yld_Vol_Dr"], "Spatial")
```
...
merge <- sp::over(grid_sp, yield_sp, fn = median)
grid_sp@data <- cbind(merge, grid_sp@data)

subplots_data <- st_as_sf(grid_sp) 
plot(subplots_data$Yld_Vol_Dr)
tm_shape(subplots_data) + tm_polygons("Yld_Vol_Dr")
...
asapplied_clean <- clean_buffer(trialutm, 30, asapplied_utm)
map_points(asapplied_utm, "Rate_Appli", "Nitrogen")

merge <- sp::over(grid_sp, as(asapplied_clean, "Spatial"), fn = median)
subplots_merge@data <- cbind(merge, subplots_merge@data)
head(subplots@data)
```
