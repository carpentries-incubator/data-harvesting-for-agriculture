...
```{r read subplot1}
subplots <- read_sf("subplots.gpkg")
...
```{r view the distribution of yield data after taking out the yield points on the boader1}
...
```{r view the distribution of cleaned yield data1}
hist(yield_clean$Yld_Vol_Dr)
```

The next line transforms the cleaned yield into UTM projection.

```{r reprojection of yield data1}
