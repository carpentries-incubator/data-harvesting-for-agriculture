...
>
> The database indicates that our seed cost per acre of corn for 2019 averaged $615.49 per acre, and that fertilizers and chemicals together come to $1,091.89 per acre.  For this exercise, we are going to simplify the model and omit equipment fuel and maintenance costs, irrigation water, and the like, and focus only on seed cost and "nitrogen".  We assume that the baseline seed rate is 37,000 seeds per acre (`seed_quo`) (although compare [this article](https://www.agry.purdue.edu/ext/corn/news/timeless/SeedingRateThoughts.html) which posits 33,000).
>
> We apply these base prices to our trial model to obtain a "seed rate" price of XXX and a "nitrogen rate" price of XXX.
>
...
>
> The USDA database indicates $4,407.75 as the baseline gross value of production for corn per acre in 2019.  Assuming the baseline yield is 1,124 bushels per acre with a price per bushel of $3.91, produce a map with the gross profit per acre indicated.
>
> > ## Solution
> >
> > ```{r sol map as pl1} XXX
> > map_gross <- tmap_arrange(map_yieldcl, map_asplanted, ncol = 2, nrow = 1)
> > map_gross
> > ```
> {: .solution}
{: .challenge}
...
>
> Calculate the difference between the cost in each grid square and the gross profit in each grid square, thus the net profit, and produce a map with the net profit per acre indicated.
>
> > ## Solution
> >
> > ```{r sol map as pl1} XXX
> > map_net <- tmap_arrange(map_yieldcl, map_asplanted, ncol = 2, nrow = 1)
> > map_net
> > ```
> {: .solution}
{: .challenge}
