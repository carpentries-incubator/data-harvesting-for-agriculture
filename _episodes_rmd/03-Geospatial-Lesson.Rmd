...
```{r transform}
trialutm <- st_transform_utm(boundary)
st_crs(boundary)
```
...
We can also plot the data where the polygons change color based on the value of one of the variables. This can be done with a package `tmap()`. We will discuss this package more in the next lesson, but we provide a function for making a simple graph of polygon features colored based on a given variable. 

```{r plotboth }
plot(boundary$geom)
tm_shape(boundary) + tm_polygons('Type') +
