...
> ```
> **DENA: Should / could they use their own data here? If so, let's tell them that. If not, can we add in an exercise where they load their own data and practice with it?**
...
> data is brought into the R environment rather than saved in your working directory. **DENA: Do we want them to save it in their working directory so they can use it again, esp. if they're using their own data?**
>
>```{r daymetr}
...
> We can use `str()` as we did in the last lesson to explore the new datmetr **DENA: is this daymetr instead?** object `weather`
> ```{r}
> str(weather)
>```
...
>
>  1. Grab this object and save it as `weather_data`. **DENA: Which "this object"? Grab it how? Do you mean highlight a specific block of text already on the scren or download something from the daymetr source collection? I can't tell from the instructions so far.**
...
>  are created from a specific R function. **DENA: This needs help -- how do they know what to look for? The letter R by itself is like the least helpful search term in history, so are there any other search assists we can give them for getting better results?**
>
> > ## Solution
> > 
> > ```{r weatherdata}
...
> > Use the `daymetr` [vignette](https://cran.r-project.org/web/packages/daymetr/vignettes/daymetr-vignette.html) to understand the meaning of these variables. **DENA: We haven't seen the word vignette before, and the definition I have of that word is very different. This is showing up in a solution, without that keyword having been spotted earlier in the text, and if this is a thing they should have googled up, I don't think they have the words to have gotten here by this point.** 
> > 
> {: .solution}
{: .challenge}
...
> or order the dataset by time. **DENA: That's a programmer's phrase -- how to say this in human? "We can look at a particular group of dates or organize them in calendar order?"** `as.Date()` converts a column to a data,  **DENA: do you mean a datE rather than a datA here?** but here if we try 
...
> of July and August. First, we need to organize the data in a new dataset with the average total rainfall for each month. **DENA: I LOVE this paragraph. It tells us up front what we're doing and why we're doing it and why that matters. Good sample! Let's use it as a model.**
{: .callout}

> # Adding a month variable
> 
> Currently, there is no month variable in our dataframe. There is a package called `lubridate`
> that can facilitate easy transformations of dates in R. **DENA: Wearing novice user hat: I thought we just did the date transformations, why are we doing different ones now with a different thing? How does this connect to the daymetr dates? Why do we need both? Where is this :: notation coming from? How am I (novice user) supposed to know when to use ::? Do I-the-student have to understand all this and keep it in my head for later, or is this just "smile and nod, it's a theoretical building block but we'll never use it again"?** We use the command `month()` to add a 
...
> `sumprec_by_monthyear()` and `avgprec_by_month()`. **Dena: Novice user thanks you! How do I-the-novice get hold of those functions again in the future? Are they in a library? Which one?**
> 
...
> before we take the average monthly precipitation. We will take the > subset **Dena: What's the greater-than subset?** of `by_month_year` that is in 2018.
> 
> ```{r monthdata}
...
a growing season. **DENA: Is this something they'd want to see how to do? If yes, are we doing it later? If not, what would it take to add an exercise where they do the thing with their data and have a shiny to look at?**
>
> Another possible graph you could make with these data is on the accumulated GDD each month. **DENA: I don't think they can get from "could do this" to "know how to do this" with these examples. Can we show them that example?**
{: .callout}
